...
(define (transitive L)
  (remove-dups (append L (transitive-closure L (* (length L)(length L)))) empty))
...
(define (transitive-closure L counter)
 (cond
   [(empty? L) empty]
   [(and (not (empty? (rest L))) (not (equal? counter 0)))
    (append (transitive-pairs (first L) L)
         (transitive-closure (transitive-pairs (first (rest L)) (rest L)) (sub1 counter)))]
   [else empty]))
...
(cond
    ...
    [(equal? (second n) (first (first L)))
     (cons (list (first n) (second (first L))) empty)
             ]
    [(equal? (first n) (second (first L)))
     (cons (list (first (first L)) (second n)) empty)
             ]
